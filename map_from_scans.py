...
def main(path, voxel_size, cpu):
    ...
    # for seq in ['08']:
        ...
        for pose, pcd_path in tqdm.tqdm(list(zip(poses, natsorted(os.listdir(os.path.join(path, seq, 'velodyne')))))):
            ...
            annotated_data = torch.from_numpy(annotated_data)
            ...
            points[:,-1] = annotated_data

            map_points = torch.cat((map_points, points[:,:]), axis=0)
            map_points_2 = map_points[:,:3]
            _, mapping = ME.utils.sparse_quantize(coordinates=map_points_2 / voxel_size, return_index=True, device=device_label)
            map_points = map_points[mapping]


        print(f'saving map for sequence {seq}')
        np.save(os.path.join(path, seq, 'map_clean2.npy'), map_points.cpu().numpy())

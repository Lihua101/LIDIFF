...
def aggregate_pcds(data_batch, data_dir, t_frame):
    ...
    for t in range(len(data_batch)):
        ...
        l_set = np.fromfile(label_file, dtype=np.uint32).reshape((-1, 1))
        l_set = l_set & 0xFFFF
        annotated_data = np.vectorize(learning_map.__getitem__)(l_set)
        ...
        p_set = np.concatenate((p_set[:,:3],one_hot_part),axis=-1)

        pose_idx = int(fname)
        p_set[:,:3] = apply_transform(p_set, poses[pose_idx])
        # p_set[:,:3] =  p_set_3
    ...
    pose_idx = int(fname)
    pcd_full[:,:3] = undo_transform(pcd_full, poses[pose_idx])
    pcd_part[:,:3] = undo_transform(pcd_part, poses[pose_idx])

...
def get_scan_completion(scan_path, path, diff_completion, max_range):
    ...
    if diff_completion is None:
        ...
        complete_scan,diff_complete_scan= diff_completion.complete_scan(points)
    ...
    return diff_pcd_pred, pcd_pred, input_points
...
def main(path, voxel_size, max_range, denoising_steps, cond_weight, diff, refine): 
    ...
    for pose, scan_path in tqdm.tqdm(random.sample(list(zip(poses, natsorted(os.listdir(f'{PATH_DATA}/velodyne')))),100)):
        ...
        # init_scan = o3d.geometry.PointCloud()
        # init_scan.points = o3d.utility.Vector3dVector(cur_scan)
        # init_scan.estimate_normals()
        # o3d.io.write_point_cloud('init_scan.ply', init_scan)

        # o3d.io.write_point_cloud('diff_pcd_pred.ply', diff_pcd_pred)

        # o3d.io.write_point_cloud('pcd_pred.ply', pcd_pred)

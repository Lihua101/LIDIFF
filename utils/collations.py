...
def point_set_to_sparse_refine(p_full, p_part, n_full, n_part, resolution, filename):
    ...
    # elif mode == 'refine':
    p_full = p_full[torch.randperm(p_full.shape[0])]
    # p_full = p_full[np.random.permutation(p_full.shape[0])]
    p_full = torch.tensor(p_full.repeat(concat_full, 0)[:n_full])   

    p_part = p_part[torch.randperm(p_part.shape[0])]
    # p_part = p_part[np.random.permutation(p_part.shape[0])]
...
def point_set_to_sparse(p_full, p_part_4, n_full, n_part, resolution, filename, p_mean=None, p_std=None):
    p_part = p_part_4[:,:3]
    concat_part = np.ceil(n_part / p_part.shape[0]) 
    p_part = p_part.repeat(concat_part, 0)
    pcd_part = o3d.geometry.PointCloud()
    pcd_part.points = o3d.utility.Vector3dVector(p_part[:,:3])
    viewpoint_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(pcd_part, voxel_size=10.)
    down_pcd_part = pcd_part.farthest_point_down_sample(n_part)
    pcd_tree = o3d.geometry.KDTreeFlann(pcd_part)
    down_pcd_part = np.array(down_pcd_part.points)
    selected_indices = []
    for point in down_pcd_part:
        _, idx, _ = pcd_tree.search_knn_vector_3d(point, 1)  # 搜索最近的点
        selected_indices.append(idx[0])
    selected_indices = np.array(selected_indices) 
    p_part = torch.tensor(p_part_4[selected_indices])
    # p_part = torch.tensor(np.array(pcd_part.points))
    
    in_viewpoint = viewpoint_grid.check_if_included(o3d.utility.Vector3dVector(p_full[:,:3]))

...
class DiffCompletion(LightningModule):
    def __init__(self, diff_path, refine_path, denoising_steps, cond_weight):
        ...
        self.partial_enc = minknet.MinkGlobalEnc(in_channels=23, out_channels=self.hparams['model']['out_dim']).cuda()
        self.model = minknet.MinkUNetDiff(in_channels=23, out_channels=self.hparams['model']['out_dim']).cuda()
    ...
    def complete_scan(self, scan):
        ...
        # numpy_array = x_feats.cpu().numpy()
        # np.save("weiquzao.npy", numpy_array)
...
def main(diff, refine, denoising_steps, cond_weight):
    ...
    # path = './lidiff/Datasets/test/'
    path = './Datasets/SemanticKITTI/dataset/sequences/001/velodyne'

...
class TemporalKITTISet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        p_concat = jitter_point_cloud(p_concat[None,:,:], sigma=.2, clip=.3)[0]

        dist_noise = np.power(p_concat[:,:3], 2)
        dist_noise = np.sqrt(dist_noise.sum(-1))

        _, mapping = ME.utils.sparse_quantize(coordinates=p_full[:,:3] / 0.1, return_index=True)
        p_full = p_full[mapping]
        dist_full = np.power(p_full[:,:3], 2)
        dist_full = np.sqrt(dist_full.sum(-1))

        return point_set_to_sparse_refine(
            p_full[dist_full < 50.],
            p_concat[dist_noise < 50.],
            self.num_points*2,
            self.num_points,
            self.resolution,

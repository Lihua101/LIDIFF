...
class DiffusionPoints(LightningModule):
    def __init__(self, hparams:dict, data_module: LightningDataModule = None):
        ...
        self.partial_enc = minknet.MinkGlobalEnc(in_channels=23, out_channels=self.hparams['model']['out_dim'])
        self.model = minknet.MinkUNetDiff(in_channels=23, out_channels=self.hparams['model']['out_dim'])
    ...
    def reset_partial_pcd(self, x_part, x_uncond, x_mean, x_std):
        x_part = self.points_to_tensor(x_part.F.reshape(x_mean.shape[0],-1,23).detach(), x_mean, x_std)
        x_uncond = self.points_to_tensor(
                torch.zeros_like(x_part.F.reshape(x_mean.shape[0],-1,23)), torch.zeros_like(x_mean), torch.zeros_like(x_std)
        )
    ...
    def forward(self, x_full, x_full_sparse, x_part, t):
        ...
        # out = self.model(x_full, x_full_sparse, x_part, t)
        torch.cuda.empty_cache()
    ...
    def points_to_tensor(self, x_feats, mean, std):
        ...
        x_coord[:,1:] = feats_to_coord(x_feats[:,1:4], self.hparams['data']['resolution'], mean, std)
    ...
    def validation_step(self, batch:dict, batch_idx):
        ...
        with torch.no_grad():
            ...
            x_gen_eval = x_gen_eval.F.reshape((gt_pts.shape[0],-1,23))
